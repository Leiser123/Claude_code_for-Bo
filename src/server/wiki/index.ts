...
import { handleChatHistoryGet, handleChatHistoryPost } from "./chat-history.js"
...
export async function handleWikiApi(req: Request, url: URL, _segments: string[]): Promise<Response> {
  ...
  // GET/POST /api/wiki/chat-history（知识库会话历史注册表，固化到 Knowledge 文件夹）
  if (resource === "chat-history") {
    if (method === "GET") return handleChatHistoryGet()
    if (method === "POST") return handleChatHistoryPost(req)
  }
