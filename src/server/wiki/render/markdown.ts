...
/**
 * Resolve a wikilink target to a file under wikiRoot.
 */
export function findPage(wikiRoot: string, target: string): string | null {
  ...
  // Try with "wiki/" prefix (files are often under a wiki/ subdirectory)
  const wikiPrefixed = tryPath(path.join("wiki", target)) || tryPath(path.join("wiki", target) + ".md")
  if (wikiPrefixed) return wikiPrefixed

  // Fallback: scan wikiRoot for matching stem.
  if (!fs.existsSync(wikiRoot)) return null
  const match = findByStem(wikiRoot, target)
  return match
}
...
function findByStem(dir: string, target: string): string | null {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      // Match by stem, full filename, or target as a path suffix
      if (stem === target || e.name === target || full.endsWith(target) || full.endsWith(target + ".md")) return full
    }
  }
  return null
}
