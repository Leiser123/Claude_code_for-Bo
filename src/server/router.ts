...
import { handleWikiApi } from './wiki/index.js'
import { handleExpertSettingsGet, handleExpertSettingsPost } from './expert-settings.js'
...
export async function handleApiRequest(req: Request, url: URL): Promise<Response> {
  ...
  switch (resource) {
    ...
    case 'wiki':
      return handleWikiApi(req, url, segments)

    case 'expert-settings':
      // GET/POST /api/expert-settings — 专家配置固化到 ~/.claude/cc-haha/expert-settings/
      if (req.method === 'GET') return handleExpertSettingsGet()
      if (req.method === 'POST') return handleExpertSettingsPost(req)
