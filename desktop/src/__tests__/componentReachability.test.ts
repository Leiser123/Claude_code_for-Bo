...
/** Paths in expectations and allowlist keys use forward slashes; normalize Windows separators. */
...
describe('component reachability', () => {
  ...
  it('finds the entry points it is supposed to walk from', () => {
    ...
    const entries = entryPoints().map((path) => normalizePath(relative(ROOT, path)))
  ...
  it('reaches every component from an entry point', () => {
    const unreachable = components
      ...
      .map((path) => normalizePath(relative(SRC, path)))
