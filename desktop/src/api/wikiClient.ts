...
// ─── Knowledge chat history API client ────────────────────────────
// 会话历史注册表固化到 ~/.claude/cc-haha/knowledge/.chat-history.json

export type KnowledgeSessionRecordApi = {
  id: string
  title: string
  createdAt: number
}
...
export const chatHistoryApi = {
  getRecords: () =>
    api.get<{ records: KnowledgeSessionRecordApi[] }>('/api/wiki/chat-history'),
  saveRecords: (records: KnowledgeSessionRecordApi[]) =>
    api.post<{ success: boolean; records: KnowledgeSessionRecordApi[] }>('/api/wiki/chat-history', { records }),
}
