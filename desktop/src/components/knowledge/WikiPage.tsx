...
export function WikiPage() {
  ...
  const handleArticleClick = (e: React.MouseEvent) => {
    ...
    // Check if it's a wikilink with data-wikilink-target
    if (target.classList.contains('wikilink')) {
      // Use href (resolved relative path) instead of data-wikilink-target (raw [[name]])
      const page = href
      if (page) {
        void loadPage(page)
    ...
    // Handle all other internal links (relative paths, ?page=, etc.)
    const page = href
    void loadPage(page)
    history.pushState({ page }, '', `/?page=${encodeURIComponent(page)}`)
  }
