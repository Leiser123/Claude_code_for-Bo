  ...
  useEffect(() => {
    ...
    sim.force('noise', () => {
      for (const n of nodes) {
        ...
        // 轻微浮动呼吸效果，幅度调小以免节点漂移导致难以点中
        n.vx = (n.vx ?? 0) + (Math.random() - 0.5) * 0.03
        n.vy = (n.vy ?? 0) + (Math.random() - 0.5) * 0.03
      }
    })
    ...
    // 透明大命中区：吸附点击，节点再小也容易点中（半径不小于 28px）
    nodeInner
      .append('circle')
      .attr('class', 'node-hit')
      .attr('r', (d) => Math.max(radius(d) + 18, 28))
    ...
    nodeSel
      ...
      .on('click', (_event, d) => {
        ...
        // 传 path 供 KnowledgeContentRouter 加载对应页面
        openTab(`wiki-${d.path}`, title, 'wiki', d.path)
      })
