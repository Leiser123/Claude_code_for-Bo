...
function SidebarToggleButton() {
  ...
  const isOutlineOpen = usePanelStore((s) => s.isOutlineOpen)
  const toggleOutline = usePanelStore((s) => s.toggleOutline)

  // project 模式没有可折叠的模块面板，隐藏按钮
  if (activeAppMode === 'project') return null

  // knowledge 模式折叠的是右侧 chat 模块面板，chat 模式折叠的是左侧会话栏
  const open = activeAppMode === 'knowledge' ? isOutlineOpen : sidebarOpen
  const onClick = activeAppMode === 'knowledge' ? toggleOutline : toggleSidebar
  const icon = open
    ? activeAppMode === 'knowledge'
      ? 'chevron_right'
      : 'chevron_left'
    : activeAppMode === 'knowledge'
      ? 'chevron_left'
