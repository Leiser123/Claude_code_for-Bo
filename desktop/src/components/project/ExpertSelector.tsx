...
export function ExpertSelector() {
  ...
  // Find which card is front-facing (closest to z=+radius)
  const findFrontIndex = useCallback(() => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      card.style.transform = `translate3d(${tx}px, 0, ${tz}px) rotateY(${-a}rad)${zLayer}`
      card.style.opacity = carouselState === 'stopped' && highlightedIndex === i ? '1' : String(opacity)
      card.style.scale = carouselState === 'stopped' && highlightedIndex === i ? '1.1' : String(scale)
      card.style.zIndex = String(zIndex)
      card.style.pointerEvents = isFront ? 'auto' : 'none'
    }
  }, [angle, experts.length, carouselState, highlightedIndex])
  ...
  const handleCardClick = (index: number) => {
    ...
    if (carouselState === 'spinning') {
      ...
      if (highlightedIndex === index) {
        setSelectedExpertId(experts[index].id)
