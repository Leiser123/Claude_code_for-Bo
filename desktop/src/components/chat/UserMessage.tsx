...
export const UserMessage = memo(function UserMessage({
  ...
  // 知识库聊天等场景会用 buildDisplayChatContent 生成结构化展示内容：开头的
  // <details> 折叠块是系统提示（默认收起、可点击展开），其后紧跟用户问题。
  // 这里拆成两块渲染：系统提示作为独立的折叠卡片放在气泡上方，用户问题单独
  // 显示在自己的气泡里（提问按纯文本渲染，`**`、`#`、文件路径保持原样）。
  const { systemPromptBlock, questionText } = useMemo(() => {
    if (!content.includes('<details>')) return { systemPromptBlock: null, questionText: content }
    const match = content.match(/^\s*<details>[\s\S]*?<\/details>/)
    if (!match) return { systemPromptBlock: null, questionText: content }
    return {
      systemPromptBlock: match[0].trim(),
      questionText: content.slice(match[0].length).replace(/^\s*\n+/, ''),
    }
  }, [content])
  ...
  const segments = useMemo(() => splitTextByUrls(questionText), [questionText])
  ...
  return (
    <div className="flex justify-end">
      ...
      >
        {systemPromptBlock && (
          // 系统提示单独一块：默认折叠、点击展开，左对齐与用户问题气泡区分开；
          // 透明弱化处理，避免与正文气泡抢视觉权重
          <div
            data-message-body="system-prompt"
            className="mb-1.5 max-w-[420px] self-start rounded-lg border border-dashed border-[var(--color-border-separator)]/60 bg-[var(--color-surface-container)]/20 px-3 py-1.5 text-xs text-[var(--color-text-tertiary)] opacity-80"
          >
            <MarkdownRenderer content={systemPromptBlock} className="[&>:first-child]:mt-0 [&>:last-child]:mb-0" />
          </div>
        )}
