...
export function StreamingIndicator({ sessionId: explicitSessionId }: { sessionId?: string | null } = {}) {
  const t = useTranslation()
  const [now, setNow] = useState(() => Date.now())
  const activeTabId = useTabStore((s) => s.activeTabId)
  // 面板（如 Knowledge 右侧聊天栏）明确传入自己的会话时，状态必须读它，
  // 不能回落到全局 activeTabId —— 那可能是另一个模式的会话，会显示错误的
  // "处理中/思考中"。
  const resolvedSessionId = explicitSessionId ?? activeTabId
