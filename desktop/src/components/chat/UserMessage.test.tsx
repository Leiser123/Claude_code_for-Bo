...
// 知识库聊天：buildDisplayChatContent 产出的结构化内容 = <details> 系统提示折叠块 +
// 用户问题。系统提示作为独立折叠卡片显示在气泡上方（默认收起、可点击展开），
// 用户问题单独显示在自己的气泡里，且按纯文本渲染。
describe('UserMessage structured display split', () => {
  it('renders the system prompt as a separate foldable block and the question as its own bubble', () => {
    const structured = '<details><summary>系统提示</summary><pre>你是知识库问答助手。\n规则：1</pre></details>\n\n什么是知识库？'
    const { container } = render(<UserMessage content={structured} />)

    const systemBlock = container.querySelector<HTMLElement>('[data-message-body="system-prompt"]')
    expect(systemBlock).toBeTruthy()
    const details = systemBlock?.querySelector('details')
    expect(details).toBeTruthy()
    expect(details?.querySelector('summary')?.textContent).toBe('系统提示')
    // 折叠块默认收起（无 open 属性），点击 summary 才能展开
    expect(details?.hasAttribute('open')).toBe(false)

    const bubble = bubbleOf(container)
    expect(bubble.textContent).toBe('什么是知识库？')
    expect(bubble.textContent).not.toContain('系统提示')
    expect(bubble.textContent).not.toContain('你是知识库问答助手。')
  })

  it('keeps a plain prompt in the bubble untouched', () => {
    const { container } = render(<UserMessage content="什么是知识库？" />)

    expect(container.querySelector('[data-message-body="system-prompt"]')).toBeNull()
    expect(bubbleOf(container).textContent).toBe('什么是知识库？')
  })
})
