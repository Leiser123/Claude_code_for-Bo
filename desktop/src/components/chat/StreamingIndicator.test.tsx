...
describe('StreamingIndicator', () => {
  ...
  it('reads the passed session instead of the global active tab', () => {
    // 全局 active tab 是一个空闲的会话；传入的 sidebar 会话正在工作。
    // 知识库右侧聊天栏等面板明确传 sessionId 时，状态必须来自它，
    // 否则会把别的模式的会话状态当成自己的"处理中"。
    useChatStore.setState({
      sessions: {
        [ACTIVE_TAB]: makeSession({ chatState: 'idle', statusVerb: '' }),
        'knowledge-panel-session': makeSession({ chatState: 'thinking', statusVerb: 'Working' }),
      },
    })

    render(<StreamingIndicator sessionId="knowledge-panel-session" />)

    expect(screen.getByTestId('turn-status-indicator').textContent).toContain('Working')
  })
})
