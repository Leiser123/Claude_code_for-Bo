...
describe('MessageList nested tool calls', () => {
  ...
  it('collapses the retrieval activity of a turn into a single searching row', () => {
    const messages: UIMessage[] = [
      { id: 'user-1', type: 'user_text', content: '粘接静电理论是什么？', timestamp: 1 },
      { id: 'think-1', type: 'thinking', content: '先定位相关页面。', timestamp: 2 },
      {
        id: 'tool-glob',
        type: 'tool_use',
        toolName: 'Glob',
      ...
      { id: 'think-2', type: 'thinking', content: '通读目录页。', timestamp: 5 },
      {
        id: 'tool-read',
        type: 'tool_use',
        toolName: 'Read',
        toolUseId: 'read-1',
        input: { file_path: 'wiki/index.md' },
        timestamp: 6,
      },
      { id: 'assistant-1', type: 'assistant_text', content: '静电理论是……', timestamp: 7 },
    ]

    const { renderItems } = buildRenderModel(messages, undefined, { mergeSearchPhase: true })

    expect(renderItems.map((item) =>
      item.kind === 'searching'
        ? `search:${item.id}`
        : item.kind === 'message'
      ...
      'message:user-1',
      'search:search-user-1',
      'message:assistant-1',
    ])
  })

  it('merges an in-progress turn (no answer yet) into one searching row', () => {
    const messages: UIMessage[] = [
      { id: 'user-1', type: 'user_text', content: '问题', timestamp: 1 },
      { id: 'think-1', type: 'thinking', content: '先定位相关页面。', timestamp: 2 },
      {
        id: 'tool-read',
        type: 'tool_use',
    ...
    expect(renderItems).toEqual([
      ...
      { kind: 'searching', id: 'search-user-1' },
    ])
  })

  it('leaves retrieval activity untouched when mergeSearchPhase is off', () => {
    const messages: UIMessage[] = [
      { id: 'user-1', type: 'user_text', content: '问题', timestamp: 1 },
      { id: 'think-1', type: 'thinking', content: '先定位相关页面。', timestamp: 2 },
      {
      ...
      { id: 'assistant-1', type: 'assistant_text', content: '答案', timestamp: 4 },
    ]

    const { renderItems } = buildRenderModel(messages)

    expect(renderItems.some((item) => item.kind === 'searching')).toBe(false)
    // 未开启合并时，检索活动以工具组形式展开（思考块保留在组内步骤里）
    expect(renderItems.some((item) => item.kind === 'tool_group')).toBe(true)
  })
