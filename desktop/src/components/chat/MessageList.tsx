...
type RenderItem =
  ...
  /**
   * Knowledge 聊天检索阶段：一轮中"提问 → 检索（思考 + 工具调用）→ 回答"的
   * 检索部分被合并成这一条淡化的"正在检索"行，避免思考块与工具调用逐条刷屏。
   */
...
export function buildRenderModel(
  ...
  options: {
    ...
    /** Knowledge 聊天：把每轮"提问 → 回答"之间的检索活动合并成一条 searching 行 */
    mergeSearchPhase?: boolean
  } = {},
): RenderModel {
  ...
  flushGroup()
  return {
    renderItems: options.mergeSearchPhase ? mergeSearchPhaseItems(items) : items,
    toolResultMap,
    childToolCallsByParent,
  }
}

/** 该渲染项是否属于"检索阶段"活动（思考块 / 工具调用 / 工具结果）。 */
function isSearchPhaseRenderItem(item: RenderItem): boolean {
  if (item.kind === 'tool_group') return true
  if (item.kind === 'message') {
    return item.message.type === 'thinking'
...
/**
 * 把每轮"用户提问之后、回答开始之前"的连续检索活动合并成一条 searching 行。
 * 一轮中检索发生在 user_text 之后，直到 assistant_text（或结束）为止；中途
 * 出现其他类型（如系统事件）则先结算当前检索段。合并后思考块和工具调用不再
 * 逐条刷屏，界面只留一条淡化的"正在检索"提示。
 */
function mergeSearchPhaseItems(items: RenderItem[]): RenderItem[] {
  const merged: RenderItem[] = []
  let pendingSearch: RenderItem[] = []
  let lastUserTextId: string | null = null
  const flushSearch = () => {
    if (pendingSearch.length === 0) return
    merged.push({
      kind: 'searching',
      id: `search-${lastUserTextId ?? getRenderItemKey(pendingSearch[0]!)}`,
    })
    pendingSearch = []
  }
  for (const item of items) {
    if (item.kind === 'message' && item.message.type === 'user_text') {
      flushSearch()
      lastUserTextId = item.message.id
      merged.push(item)
      continue
    }
    ...
    if (isSearchPhaseRenderItem(item)) {
      pendingSearch.push(item)
      continue
    }
    flushSearch()
    merged.push(item)
...
type MessageListProps = {
  ...
  /** Knowledge 聊天：把每轮提问后的检索活动合并成一条淡化的"正在检索"行 */
  mergeSearchPhase?: boolean
}
...
function getRenderItemKey(item: RenderItem) {
  if (item.kind === 'tool_group' || item.kind === 'team_card' || item.kind === 'searching') return item.id
...
function getRenderItemContentWeight(item: RenderItem): number {
  ...
  if (item.kind === 'team_card' || item.kind === 'searching') return 0
...
/** Knowledge 聊天检索合并行：单行淡化的"正在检索"提示。 */
const SEARCH_PHASE_ROW_HEIGHT = 32
...
export function estimateRenderItemHeight(item: RenderItem): number {
  ...
  if (item.kind === 'searching') return SEARCH_PHASE_ROW_HEIGHT
...
function getRenderItemMetricSignature(item: RenderItem): string {
  ...
  if (item.kind === 'searching') return `searching:${item.id}`
  if (item.kind === 'team_card') {
    ...
    return (
      <>
        {item.kind === 'tool_group' ? (
          ...
          <div
            data-testid="search-phase-row"
            className="my-1 flex items-center gap-1.5 px-1 text-xs text-[var(--color-text-tertiary)] opacity-60"
          >
            <span
        ...
        >
          ...
          {showsTurnStatusLine && (
            ...
            <div className={renderItems.length === 0 ? undefined : turnRailClass('end')}>
              <StreamingIndicator sessionId={resolvedSessionId} />
            </div>
          )}
