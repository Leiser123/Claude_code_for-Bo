...
/** Windows separators are backslashes; comparisons and messages use forward slashes. */
...
describe('css custom property usage', () => {
  it('resolves every var(--token) referenced from components', () => {
    ...
    for (const file of sourceFiles) {
      ...
      // 统一用正斜杠比较，否则 Windows 反斜杠路径永远匹配不上。
      if (normalizePath(file).includes('/preview-agent/')) continue
