...
export const useWikiStore = create<WikiState>((set, get) => ({
  ...
  fetchRepos: async () => {
    try {
      ...
      const wikis: WikiInfo[] = wikisData.wikis.map((w) => ({
        name: w.name,
        path: w.path,
      }))
      const currentRepo = wikis.find((r) => r.name === cfg.wikiRoot) || wikis[0] || null
      const repos: RepoItem[] = wikis.map((w) => ({
        ...w,
        status: (w.name === currentRepo?.name ? 'active' : 'inactive') as 'active' | 'inactive',
      }))
      set({
        repos,
        currentRepo,
  ...
  switchWiki: async (name, path?) => {
    try {
      const result = await wikiApi.switchWiki(name, path)
      set({
