...
describe('chatStore appendAssistantText', () => {
  beforeEach(() => {
    localStorage.clear()
    useChatStore.setState({
      ...initialState,
      sessions: {
        [TEST_SESSION_ID]: makeSession({
          messages: [
            { id: 'u1', type: 'user_text', content: '问题', timestamp: 1 },
            { id: 'a1', type: 'assistant_text', content: '回答', timestamp: 2, transcriptMessageId: 'ta-1' },
          ],
        }),
      },
    })
  ...
  it('appends references into the last assistant reply when the transcript id matches', () => {
    useChatStore.getState().appendAssistantText(TEST_SESSION_ID, '\n\n**参考依据**\n\n[1] <a href="#">x</a>', 'ta-1')
    ...
    expect(messages[1]).toMatchObject({
      type: 'assistant_text',
      content: '回答\n\n**参考依据**\n\n[1] <a href="#">x</a>',
      transcriptMessageId: 'ta-1',
    })
  })

  it('starts a new assistant message when the transcript id does not match', () => {
    useChatStore.getState().appendAssistantText(TEST_SESSION_ID, ' 追加文本  ', 'ta-other')

    const messages = useChatStore.getState().sessions[TEST_SESSION_ID]?.messages ?? []
    expect(messages).toHaveLength(3)
    expect(messages[2]).toMatchObject({ type: 'assistant_text', content: ' 追加文本  ' })
  })

  it('is a no-op for an unknown session', () => {
    useChatStore.getState().appendAssistantText('missing-session', '文本')
    expect(useChatStore.getState().sessions['missing-session']).toBeUndefined()
  })
})

describe('chatStore replaceAssistantText', () => {
  beforeEach(() => {
    useChatStore.setState({
      ...initialState,
      sessions: {
  ...
  it('replaces the assistant message content when the transcript id matches', () => {
    useChatStore.getState().replaceAssistantText(TEST_SESSION_ID, 'ta-1', '正文\n\n**参考依据**\n\n[1] <a href="#">x</a>')
  ...
  it('replaces the last assistant message when no transcript id is given', () => {
    useChatStore.getState().replaceAssistantText(TEST_SESSION_ID, undefined, '被替换的内容')

    const messages = useChatStore.getState().sessions[TEST_SESSION_ID]?.messages ?? []
    expect(messages).toHaveLength(2)
    expect(messages[1]).toMatchObject({ type: 'assistant_text', content: '被替换的内容' })
  })

  it('is a no-op when there is no assistant message to replace', () => {
    useChatStore.setState({
      ...initialState,
      sessions: {
        [TEST_SESSION_ID]: makeSession({
          messages: [{ id: 'u1', type: 'user_text', content: '问题', timestamp: 1 }],
        }),
      },
    })
    useChatStore.getState().replaceAssistantText(TEST_SESSION_ID, undefined, '文本')

    const messages = useChatStore.getState().sessions[TEST_SESSION_ID]?.messages ?? []
    expect(messages).toHaveLength(1)
    expect(messages[0]).toMatchObject({ type: 'user_text', content: '问题' })
  })
})
