...
type ChatStore = {
  ...
  /** 向会话最后一条 assistant 回复末尾追加文本（用于追加"参考依据"等系统生成内容）。 */
  appendAssistantText: (sessionId: string, text: string, transcriptMessageId?: string) => void
  /** 替换某条 assistant 消息的完整内容（无 transcriptMessageId 时替换最后一条）。 */
  replaceAssistantText: (sessionId: string, transcriptMessageId: string | undefined, content: string) => void
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  appendAssistantText: (sessionId, text, transcriptMessageId) => {
    set((state) => {
      const session = state.sessions[sessionId]
      if (!session) return state
      const messages = appendAssistantTextMessage(
        session.messages,
        text,
        Date.now(),
        undefined,
      ...
      if (messages === session.messages) return state
      return { sessions: updateSessionIn(state.sessions, sessionId, () => ({ messages })) }
    })
  },

  replaceAssistantText: (sessionId, transcriptMessageId, content) => {
    set((state) => {
      const session = state.sessions[sessionId]
      if (!session) return state
      const messages = [...session.messages]
      ...
      if (transcriptMessageId) {
        targetIndex = messages.findIndex(
          (m) => m.type === 'assistant_text' && m.transcriptMessageId === transcriptMessageId,
        )
      } else {
        for (let i = messages.length - 1; i >= 0; i -= 1) {
          if (messages[i]?.type === 'assistant_text') {
            targetIndex = i
            break
          }
        }
      ...
      if (target.type !== 'assistant_text') return state
      if (target.content === content) return state
      messages[targetIndex] = { ...target, content }
      return { sessions: updateSessionIn(state.sessions, sessionId, () => ({ messages })) }
    })
  },
