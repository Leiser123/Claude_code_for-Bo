...
import { useExpertSessionsStore } from './expertSessionsStore'
import { useSettingsStore } from './settingsStore'
import { useTabStore } from './tabStore'
import type { LocalIndexStatus, SessionListItem, SessionMode } from '../types/session'
...
type SessionStore = {
  ...
  getSessionsByMode: (mode: SessionMode) => SessionListItem[]
...
export const useSessionStore = create<SessionStore>((set, get) => ({
  ...
  getSessionsByMode: (mode) => {
    const { sessions } = get()
    if (mode === 'chat') {
      return sessions.filter((s) => !s.mode || s.mode === 'chat')
    }
  ...
  createSession: async (workDir?: string, options?: CreateSessionOptions) => {
    ...
    const sessionMode = options?.mode
    set((state) => ({
      sessions: state.sessions.some((session) => session.id === id)
        ? state.sessions
        : [optimisticSession, ...state.sessions],
      activeSessionId: sessionMode && sessionMode !== 'chat' ? state.activeSessionId : id,
    }))
...
function preserveLocalTitle(
  ...
  // mode 是前端本地字段（服务端列表不返回）。知识库/专家会话在本地注册表登记，
  // 合并时强制恢复对应 mode，避免重启后从服务端拉取时漏进 chat 侧栏。
  const registeredAsKnowledge = useKnowledgeSessionsStore
    .getState()
    .sessions.some((entry) => entry.id === incoming.id)
  const registeredAsExpert = useExpertSessionsStore
    .getState()
    .sessions.some((entry) => entry.id === incoming.id)
  const restoredMode = registeredAsKnowledge
    ? 'knowledge'
    : registeredAsExpert
      ? 'expert'
